...
import datetime
import sys
import requests  # 引入网络库
import threading  # 引入多线程库(防止界面卡死)
...
class ExcelManager:
    ...
    def get_month_data(self):
        default_res = (0.0, 0.0, "暂无数据")
        if not os.path.exists(self.filepath): return default_res
        trans, _, config = self.read_data()
        if trans is None or trans.empty: return (0.0, 0.0, "暂无流水")
        try:
            trans['日期'] = pd.to_datetime(trans['日期'])
        except:
            return (0.0, 0.0, "日期格式错误")
        now = datetime.datetime.now()
        df = trans[(trans['日期'].dt.year == now.year) & (trans['日期'].dt.month == now.month)].copy()
        if df.empty: return (0.0, 0.0, "本月暂无流水")
        df['币种'] = df['币种'].astype(str)
        if config is not None:
            config['币种'] = config['币种'].astype(str)
            if '换汇汇率' in df.columns: df = df.drop(columns=['换汇汇率'])
            df = df.merge(config[['币种', '换汇汇率']], on='币种', how='left')
            df['换汇汇率'] = df['换汇汇率'].fillna(1)
        else:
            df['换汇汇率'] = 1
        df['折合金额'] = df['金额'] * df['换汇汇率']
        total_exp = df[df['类型'].isin(['支出', '转出'])]['折合金额'].sum()
        total_inc = df[df['类型'].isin(['收入', '转入', '余额调整'])]['折合金额'].sum()
...
class RateConfigWindow(ctk.CTkToplevel):
    ...
    def fetch_online_rates(self):
        try:
            # 使用免费的 ExchangeRate-API (以 CNY 为基准)
            # URL 返回: 1 CNY = X Foreign Currency
            url = "https://api.exchangerate-api.com/v4/latest/CNY"
            response = requests.get(url, timeout=5)
            data = response.json()

            rates = data.get('rates', {})
            # 这里的 rates 是: 1 CNY = 0.138 USD
            # 我们需要的是: 1 USD = ? CNY (即 1 / 0.138)

            updated_count = 0
            for curr, entry in self.entries.items():
                if curr in rates:
                    rate_from_api = rates[curr]  # 1 CNY = X USD
                    if rate_from_api > 0:
                        my_rate = 1.0 / rate_from_api  # 转换成 1 USD = X CNY

                        # 特殊处理 MOP (如果没有 MOP，通常可以用 HKD 近似，或者 API 里有)
                        # API 应该包含 MOP。
            ...
            self.status_lbl.configure(text=f"✅ 成功更新 {updated_count} 个币种汇率", text_color="green")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.title("MoneyApp v11.0 Online")
        ...
        stats.pack(fill="x", padx=25)
        self.exp_lbl = ctk.CTkLabel(stats, text="支出: 0", text_color="#FF4D4D", font=("bold", 13))
        self.exp_lbl.pack(side="left")
        self.inc_lbl = ctk.CTkLabel(stats, text="收入: 0", text_color="#2CC985", font=("bold", 13))
        self.inc_lbl.pack(side="right")
    ...
    def refresh_ui(self):
        ...
        exp, inc, txt = self.excel_mgr.get_month_data()
        self.exp_lbl.configure(text=f"本月支出: ¥ {exp:,.2f}")
        self.inc_lbl.configure(text=f"本月收入: ¥ {inc:,.2f}")
